...
def set_outputs_from_tasks(n_epoch, tasks, objsets, out_pnt_xy, out_word, mask_pnt, mask_word):
    j = 0
...
def set_outputs_from_targets(n_epoch, objsets, out_pnt_xy, out_word, mask_pnt, mask_word):
    j = 0
    for epoch_now in range(n_epoch):
...
def generate_batch(tasks, n_epoch=30, img_size=224, objsets=None, n_distractor=1, average_memory_span=2):
    ...
    if objsets is None:
        ...
        for task in tasks:
            objsets.append(
                task.generate_objset(n_epoch, n_distractor=n_distractor, average_memory_span=average_memory_span)
            )
    ...
    if isinstance(objsets[0], sg.StaticObjectSet):
        set_outputs_from_targets(n_epoch, objsets, out_pnt_xy, out_word, mask_pnt, mask_word)
    else:
        set_outputs_from_tasks(n_epoch, tasks, objsets, out_pnt_xy, out_word, mask_pnt, mask_word)
    ...
    return (in_imgs, in_rule, seq_length, out_pnt, out_pnt_xy, out_word, mask_pnt, mask_word)
...
def static_objsets_from_examples(examples):
    ...
    static_objsets = []
    for e in examples:
        static_objs = [o for multi_epoch_obj in e['objects'] for o in sg.static_objects_from_dict(multi_epoch_obj)]
        static_objset = sg.StaticObjectSet(n_epoch=e['epochs'], static_objects=static_objs, targets=e['answers'])
...
def json_to_feeds(json_examples):
    ...
    values = generate_batch(
        rules,
        n_epoch=epochs,
        img_size=112,
        objsets=static_objsets,
