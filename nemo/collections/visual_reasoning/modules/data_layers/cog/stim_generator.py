...
#
# ==============================================================================
# COG code comes from https://github.com/IBM/mi-prometheus, based itself on
# code that was originally taken from https://github.com/google/cog :
# ==============================================================================
#
...
class Loc(Attribute):
    ...
    def get_opposite_space_to(self, space_type):
        opposite_space = {'left': 'right', 'right': 'left', 'top': 'bottom', 'bottom': 'top',}[space_type]
...
class Space(Attribute):
    ...
    def sample(self, avoid=None):
        ...
        for i_try in range(n_max_try):
            ...
            loc = (round(random.uniform(*xrange), 3), round(random.uniform(*yrange), 3))

            not_overlapping = True
            for loc_avoid in avoid:
                not_overlapping *= (loc[0] - loc_avoid[0]) ** 2 + (loc[1] - loc_avoid[1]) ** 2 > avoid_radius2
    ...
    def include(self, loc):
        ...
        x, y = loc.value
        return (self._value[0][0] < x < self._value[0][1]) and (self._value[1][0] < y < self._value[1][1])
...
def static_objects_from_dict(d):
  ...
  epochs = d['epochs'] 
    ...
    return [StaticObject(loc=tuple(d['location']), color=d['color'], shape=d['shape'], epoch=e) for e in epochs]
...
class Object(object):
    ...
    def __init__(self, attrs=None, when=None, deletable=False):
    ...
    def dump(self):
        ...
        return {
            ...
            'epochs': (self.epoch[0] if self.epoch[0] + 1 == self.epoch[1] else list(range(*self.epoch))),
    ...
    def to_static(self):
        ...
        return [
            StaticObject(loc=self.loc.value, color=self.color.value, shape=self.shape.value, epoch=epoch)
...
class ObjectSet(object):
    ...
    def add(self, obj, epoch_now, add_if_exist=False, delete_if_can=True):
        """Add an object.
    ...
    def select(self, epoch_now, space=None, color=None, shape=None, when=None, n_backtrack=None, delete_if_can=True):
    ...
    def select_now(self, epoch_now, space=None, color=None, shape=None, delete_if_can=False):
...
def render_static_obj(canvas, obj, img_size):
    ...
    if shape == 'circle':
        ...
        pts = np.array(
            [(center[0], center[1] - r1), (center[0] - r3, center[1] + r2), (center[0] + r3, center[1] + r2)]
        )
...
def render_static(objlists, img_size=224):
    ...
    for objects in objlists:
        ...
        epoch_obj_dict = defaultdict(list, [(epoch, list(group)) for epoch, group in itertools.groupby(objects, key)])
    ...
    movie = np.zeros((len(objlists) * n_epoch_max, img_size, img_size, 3), np.float32)
...
def render_target(movie, target):
    ...
    for frame, target_now in zip(movie_withtarget, target):
        if isinstance(target_now, Loc):
            ...
            draw = ImageDraw.Draw(image)
            draw.ellipse(
                (center[0] - radius, center[1] - radius, center[0] + radius, center[1] + radius), fill=(255, 255, 255)
            )
...
def sample_colorshape(k):
    return [(Color(c), Shape(s)) for c, s in random.sample(const.ALLCOLORSHAPES, k)]


def n_sample_colorshape(k):
    return np.prod(range(len(const.ALLCOLORSHAPES) - k + 1, len(const.ALLCOLORSHAPES) + 1))
...
def another_loc(space):
    n_max_try = 100
    for i_try in range(n_max_try):
        loc = Loc((round(random.uniform(0.05, 0.95), 3), round(random.uniform(0.05, 0.95), 3)))
