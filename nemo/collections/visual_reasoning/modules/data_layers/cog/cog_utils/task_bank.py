...
class GetColorSpace(Task):
    ...
    def __init__(self):
        ...
        loc = tg.GetLoc(objs1)
        objs2 = tg.Select(loc=loc, shape=shape2, when=sg.random_when(), space_type=sg.random_space())
...
class GetShapeSpace(Task):
    ...
    def __init__(self):
        ...
        loc = tg.GetLoc(objs1)
        objs2 = tg.Select(loc=loc, color=color2, when=sg.random_when(), space_type=sg.random_space())
...
class ExistColorSpace(Task):
    ...
    def __init__(self):
        ...
        objs1 = tg.Select(color=color1, when=sg.random_when())
        loc = tg.GetLoc(objs1)
        objs2 = tg.Select(loc=loc, color=color2, when='now', space_type=sg.random_space())
...
class ExistShapeSpace(Task):
    ...
    def __init__(self):
        ...
        objs1 = tg.Select(shape=shape1, when=sg.random_when())
        loc = tg.GetLoc(objs1)
        objs2 = tg.Select(loc=loc, shape=shape2, when='now', space_type=sg.random_space())
        self._operator = tg.Exist(objs2)
...
class ExistSpace(Task):
    ...
    def __init__(self):
        ...
        loc = tg.GetLoc(objs1)
        objs2 = tg.Select(loc=loc, color=attr2[0], shape=attr2[1], when='now', space_type=sg.random_space())
        self._operator = tg.Exist(objs2)
...
class SimpleExistColorGo(Task):
    ...
    def __init__(self):
        ...
        self._operator = tg.Switch(tg.Exist(objs1), tg.Go(objs1), tg.Go(objs2), both_options_avail=False)
...
class AndSimpleExistColorGo(Task):
    ...
    def __init__(self):
        ...
        self._operator = tg.Switch(bool1, tg.Go(objs1), tg.Go(objs3), both_options_avail=False)
...
class ExistColorGo(Task):
    ...
    def __init__(self):
        ...
        objs2 = tg.Select(color=color2, when=sg.random_when())
        objs3 = tg.Select(color=color3, when=sg.random_when())
        self._operator = tg.Switch(tg.Exist(objs1), tg.Go(objs2), tg.Go(objs3), both_options_avail=False)
...
class AndSimpleCompareColor(Task):
    ...
    @property
    def instance_size(self):
        return sg.n_sample_shape(2) * sg.n_random_when() ** 2 * sg.n_random_color() ** 2 * 4
...
class AndSimpleCompareShape(Task):
    ...
    @property
    def instance_size(self):
        return sg.n_sample_color(2) * sg.n_random_when() ** 2 * sg.n_random_shape() ** 2 * 4
...
class SimpleCompareColorGo(Task):
    ...
    @property
    def instance_size(self):
        return (sg.n_sample_shape(2) * sg.n_random_color() * sg.n_sample_color(2) * sg.n_random_when()) * 2
...
class SimpleCompareShapeGo(Task):
    ...
    @property
    def instance_size(self):
        return (sg.n_sample_color(2) * sg.n_random_shape() * sg.n_sample_shape(2) * sg.n_random_when()) * 2
