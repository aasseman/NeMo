...
def generate_example(memory_length, max_distractors, task_family, epochs):
    ...
    objset = task.generate_objset(
        n_epoch=epochs, n_distractor=random.randint(1, max_distractors), average_memory_span=avg_mem
    )
...
class FileWriter(object):
    ...
    def close(self):
        ...
        if self.compress:
            ...
            with open(self._file_name(), 'rb') as f_in, gzip.open(self._file_name() + '.gz', 'wb') as f_out:
...
def generate_dataset(
    ...
    if random_families:
        ...
        f = FileWriter(base_name=base_fname, per_file=per_file, start_index=start_index, compress=compress)
...
def main(path, examples_per_task, sequence_length, memory_length, max_distractors, nr_processors):
    ...
    if nr_processors > 2:
        ...
        for i in range_fn(train_parallel):
            jobs.append(
                pool.apply_async(
                    ...
                    (
                        path,
                        examples_per_task_per_job,
                        sequence_length,
                        memory_length,
                        max_distractors,
                        cog_variant,
                        2 * i,
                        n_tasks * examples_per_task_per_job // 2,
                    ),
                )
            )
        jobs.append(
            pool.apply_async(
                generate_val_or_test,
                (
                    path,
                    examples_per_task,
                    sequence_length,
        ...
        generate_val_or_test(
            path, examples_per_task, sequence_length, memory_length, max_distractors, nr_processors, 'val', cog_variant
        )
        generate_val_or_test(
            path,
            examples_per_task,
